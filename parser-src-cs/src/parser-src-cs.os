...
Функция ПолучитьМетодыСОписанием() Экспорт
    ...
    Для каждого Сп Из Совпадения Цикл
        ...
        Сообщить("БАЗА: " + ОбработкаВставленногоМетода + " :: " + СокрЛП(СпГр[0].Значение));
        ...
        Если (Значение15 <> "") Тогда
            ...
            Для каждого Элемент Из МассивПараметры Цикл
                ...
                Если СтруктураПараметра.Количество() > 2 Тогда
        ...
        Если ЭтоЭкспортноеИмя = Истина Тогда
            ...
            СтруктураЧлена.Вставить("Возврат", МассивВозвратов);
...
Для каждого Стр Из МассивЧленов Цикл
    Сообщить(Стр.ИмяРус + " / " + Стр.ИмяАнг);
    Сообщить("	Описание: ");
    Сообщить("		" + Стр.Описание);
    Если Стр.Возврат <> Неопределено Тогда
        Сообщить("	Возврат: ");
        Для каждого СтрВ Из Стр.Возврат Цикл
            Сообщить("		" + СтрВ);
        КонецЦикла;
    КонецЕсли;
