...
Функция ОбрататьТекст(ИмяФайла)
    ...
    РВ = Новый РегулярноеВыражение("((?s)(?<=\<summary\>)(.*?)(?=\<\/summary\>))|((?s)(?<=\<param\s+name=\""(.*?)\""\>)(.*?)(?=\<\/param\>))|((?s)(?<=\<returns\>)(.*?)(?=\<\/returns\>))|(\[)(ContextClass|ContextMethod|ContextProperty|ScriptConstructor)(\((\""(.*)\""(\s*),(\s*)\""(.*)\"")\))(\])|(^\s*(public)\s+(.*)\s+(.*)((?s)(?<=\((.*?)(?=\\))))|(\s+(\bget\b|\bset\b))");
