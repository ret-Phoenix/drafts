...
Процедура Тест_Должен_ПолучитьМетодыМодуля2() Экспорт
	...
	Парсер = Новый ПарсерМодуля;
	Парсер.УстановитьФайлМодуля("tests/module.os");
	Результат = Парсер.ПолучитьМетодыСОписанием();
	...
	Для каждого Метод Из Результат Цикл
		...
		Если Метод.ВозвращаемоеЗначение = Неопределено Тогда
			Сообщить("ВозвращаемоеЗначение: Неопределены");
		Иначе
			Для каждого Параметр Из Метод.ВозвращаемоеЗначение Цикл
				Сообщить("Возврат: " + Параметр.Ключ + " - " + Параметр.Значение);
				//Сообщить("Параметры: 0: "+ Параметр.Имя + " 1: " + Параметр.ПоЗначению + " 2: " + Параметр.ЗначениеПоУмолчанию + " 3: " + Параметр.Описание );
			КонецЦикла;
		КонецЕсли;
